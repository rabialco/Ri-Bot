...
public interface LineUserRepository extends JpaRepository<LineUser, String> {
    @Query(value = "SELECT * FROM LINE_USER2 WHERE USER_ID = ?1", nativeQuery = true)
    LineUser findLineUserByUserId(String userID);

    @Query(value = "SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END "
           + "FROM LINE_USER2 c WHERE c.USER_ID = ?1", nativeQuery = true)
    boolean isUserRegistered(String userID);

    @Query(value = "SELECT state FROM LINE_USER2 WHERE USER_ID = ?1", nativeQuery = true)
